...
\title{Dynamic Simulations of Autoregressive Relationships in \proglang{R} with dynsim}
...
\Abstract{
    This code snippet is a short introduction the \pkg{dynsim} \proglang{R} package for calculating dynamic simulations with autoregressive time series using the approach laid out in \cite{WilliamsWhitten2012}. \code{dynsim} depicts long-run simulations of dynamic processes for a variety of substantively interesting scenarios, with and without the presence of exogenous shocks. The package includes the \code{dynsim} function for calculating the dynamic simulations and provides a function (\code{dynsimGG}) for graphically depicting the dynamic simulations via the \code{ggplot2} package.
}
...
\begin{enumerate}
  \item Estimate your linear model using the core \proglang{R} function \code{lm}.
  \item Set up starting values for simulation scenarios and (optionally) shock values at particular iterations (e.g. points in forecasted time).
...
The output from \code{dynsimGG} is generally a \pkg{ggplot2} \code{gg} class object.\footnote{This is not true if you include a shock plot.} Because of this you can further change the aesthetic qualities of the plot using any relevant function from \pkg{ggplot2}.

\subsection{Alternative Packages in R}

The \pkg{forecast} package \citep{forecast-jss,R-forecast} provides a variety of automatic forecasting algorithms using both exponential smoothing and ARIMA models and includes plotting capabilities. While certainly impressive in its breadth of models available, our function differs in two important ways for the practical researcher.  First, \pkg{forecast} relies on an algorithm to find the appropriate time series specification, whereas our analysis assumes that scholars have already identified the appropriate model, most often through a combination of theory and model building steps.  Second, the focus of \pkg{forecast} is on developing an accurate forecast given the particular attributes of the time series.  \pkg{dynsim}, on the other hand, concentrates its efforts on exploring the causal effects of the explanatory variables. This is accomplished by establishing various theoretically-interesting scenarios, and allowing for exogenous shocks to the series.  Thus, we think that \pkg{dynsim} offers a number of distinct advantages for the practical researcher.

\section{Examples}

To get a better sense of how \pkg{dynsim} works, let's go through a few examples. We will use the \cite{Grunfeld1958} data set. It is included with \pkg{dynsim}. To load the data use:
...
Now that we have created our lag variable we, can begin to create dynamic simulations with \pkg{dynsim} by estimating the underlying linear regression model using \code{lm}, i.e.:
...
\noindent In this example we won't introduce shocks, so now we run our simulations:

<<tidy=FALSE, echo=TRUE, message=FALSE, warning=FALSE>>=
Sim1 <- dynsim(obj = M1, ldv = "InvestLag", scen = ScenComb, n = 20)
@

\noindent This took 1000 draws of the coefficients to produce 20 dynamic simulation iterations for each of the three scenarios. In the resulting \code{Sim1} object we have retained the middle 95\% and 50\%. We will return to presenting and exploring the contents of \code{Sim1} later in this section.
...
\noindent Now we can simply add \code{grunfeldshock} to the \code{dynsim} \code{shocks} argument.
...
Probably the easiest and most effective way to communicate your \code{dynsim} simulation results is with the package's built-in plotting capabilities. These are accessed with the \code{dysnsimGG} function. Let's start by plotting the simulations from our basic model. In an earlier example we put these results into \code{Sim1}. Using \code{dynsimGG}'s default settings we create Figure~\ref{defaultPlot} with the following code:
...
\noindent We can make a number of aesthetic changes. For example, we can re-title the legend using the \code{leg.name} argument and change the legend values by adding a vector of the desired label names in the order of our scenarios using the \code{leg.labels} argument. We can also change the color palette using the \code{color} argument. Let's use the `orange-red' palette denoted by \code{OrRd}. Finally, we can change the y-axis label with the \code{ylab} argument so that it is more descriptive. Adding these changes using the following code creates Figure~\ref{noShock}.
...
\begin{figure}
    \begin{center}
<<SimDefault-eval, tidy=FALSE, echo=FALSE, message=FALSE, fig.height=4, fig.width=6>>=
dynsimGG(Sim1)
@
    \end{center}
        \caption{Default Dynamic Simulation Plot with Three Scenarios}
        \label{defaultPlot}
\end{figure}

\begin{figure}
    \begin{center}
<<Sim1Labels-eval,tidy=FALSE, echo=FALSE, message=FALSE, fig.height=4, fig.width=6>>=
Labels <- c("95th Percentile", "Mean", "5th Percentile")
...
\noindent Similarly, we can plot the simulations from scenarios where market value shocks were interacted with capital stock. This can be seen in Figure \ref{shockplotInteract}. The code is almost identical to the code chunk we just saw. The only difference is that we use the \code{Sim3} model object.

<<Sim3Shock, tidy=FALSE, eval=FALSE>>=
...
\begin{figure}
    \begin{center}
<<Sim3Shock-eval, tidy=FALSE, echo=FALSE, fig.height=5, fig.width=6>>=
dynsimGG(Sim3, leg.name = "Scenarios", leg.labels = Labels, color = "OrRd",
       ylab = "Predicted Real Gross Investment\n", shockplot.var = "mvalue",
       shockplot.ylab = "Firm Value",
       shockplot.heights = c(8, 4))
@
    \end{center}
    \caption{An Example of a Dynamic Simulation Where a Shock Variable Is Interacted with an Exogenous Variable}
    \label{shockplotInteract}
\end{figure}

\section{Summary}

In this code snippet we have demonstrated how the \proglang{R} package \pkg{dynsim} makes it easy implement Williams and Whitten's \citeyearpar{WilliamsWhitten2012} approach to more completely interpreting results from autoregressive time-series models where the effects of explanatory variables have both short- and long-term components. Hopefully, this will lead to more meaningful investigations and more useful presentations of results estimated from these relationships.
